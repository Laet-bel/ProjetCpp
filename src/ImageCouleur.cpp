...
CImageCouleur::CImageCouleur(const CImageNdg& image) {

	this->m_iHauteur = image.lireHauteur();
	this->m_iLargeur = image.lireLargeur();
	this->m_sNom = "inconnu";

	this->m_pucData = new unsigned char[m_iHauteur * m_iLargeur * 3];
	this->m_ppucPixel = new unsigned char* [m_iHauteur * m_iLargeur];
	for (int i = 0; i < m_iHauteur * m_iLargeur; i++)
		this->m_ppucPixel[i] = &this->m_pucData[3 * i];

	for (int i = 0; i < this->lireNbPixels(); i++)
	{
		this->m_ppucPixel[i][0] = image(i);
		this->m_ppucPixel[i][1] = image(i);
		this->m_ppucPixel[i][2] = image(i);
	}
}
